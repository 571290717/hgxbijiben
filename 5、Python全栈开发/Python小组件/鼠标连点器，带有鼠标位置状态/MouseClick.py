...
class MouseClick:
    ...
    def mouse_click(self):
            ...
            key_listener.start()
            while self.running:
                state2.insert(INSERT, f'当前位置鼠标位置：{pyautogui.position()}\n')
                state2.insert(INSERT, f'当前时间间隔：{self.time}秒\n')
                self.mouse.click(self.button)  # 模拟鼠标点击
                time.sleep(self.time)  # 间隔一定时间
                state2.delete('0.0', END)
            key_listener.stop()
...
state2.place(relx=0.05, y=200, relwidth=0.9, height=60)
