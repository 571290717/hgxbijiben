...
def pdfload(text_name,pdfhref):
    ...
    try:
        ...
        with open('/workspace/programming-language-demo/测试床/111111报错了请手动输入名字.pdf', 'wb') as f:
            f.write(response.content)
...
for i in range(1,8):
    # print(i)
    # 目标url
    url = f'https://aii-alliance.org/index/c147.html?page={i}'
    print(url)
    ...
    for i in href_list:
        ...
        text_name = f"/workspace/programming-language-demo/测试床/{str(title_list[0])}.pdf"
        # text_name2 = f"/workspace/programming-language-demo/测试床/'{str(title_list[0])}'.pdf"
